...
export default defineNuxtModule({
  ...
  async setup(moduleOptions, nuxt) {
    ...
    const cssPath =
      moduleOptions.cssPath && resolveAlias(moduleOptions.cssPath);
    const injectPosition = ~~Math.min(
      moduleOptions.injectPosition,
      (nuxt.options.css || []).length + 1
    );
    ...
    if (typeof cssPath === "string") {
      if (existsSync(cssPath)) {
        logger.info(
          `Using Tailwind CSS from ~/${relative(nuxt.options.srcDir, cssPath)}`
        );
        nuxt.options.css.splice(injectPosition, 0, cssPath);
      } else {
        const resolver = createResolver(import.meta.url);
        nuxt.options.css.splice(
          injectPosition,
          0,
    ...
    if (existsSync(configPath)) {
      ...
      logger.info(
        `Merging Tailwind config from ~/${relative(
          nuxt.options.srcDir,
          configPath
        )}`
      );
    ...
    if (moduleOptions.exposeConfig) {
      const resolveConfig = await import("tailwindcss/resolveConfig.js").then(
        (r) => r.default || r
      );
    ...
    postcssOptions.plugins["tailwindcss/nesting"] =
      postcssOptions.plugins["tailwindcss/nesting"] ?? {};
