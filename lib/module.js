...
const extend = defu.extend((obj, key, val) => {
  if (Array.isArray(val) && typeof obj[key] === 'function') {
    obj[key] = obj[key](val)
    return true
  }
})
...
module.exports = async function (moduleOptions) {
  ...
  const options = extend(moduleOptions, nuxt.options.tailwindcss, {
  ...
  if (await pathExists(cssPath)) {
    logger.info(`Using Tailwind CSS from ~/${relative(nuxt.options.rootDir, cssPath)}`)
  ...
  tailwindConfig = extend(tailwindConfig, options.config)
