...
module.exports = async function (moduleOptions) {
  ...
  nuxt.hook('build:before', async () => {
    ...
    if (options.windicss) {
      ...
      if (windicssOptions) {
        // @todo maybe tell the user we have removed these if they have them set
        delete tailwindConfig.variants
        ...
        try {
          ...
          logger.warn('Please add the `vue-windi-css-preprocess` package to use windicss.')
          logger.info('yarn add -D vue-windi-css-preprocess')
          return
        }
        extendBuild((config) => {
          ...
          config.module.rules.push({
            // @todo check this pre flag is correct
            enforce: 'pre',
            test: /\.vue$/,
